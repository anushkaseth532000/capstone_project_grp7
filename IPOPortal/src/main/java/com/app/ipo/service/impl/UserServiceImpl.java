	...
	{		
				...
				&& (null != userDto.getUserPassword() && !userDto.getUserPassword().equalsIgnoreCase("")))
		{
			if(null != userDto && userDto.getCategoryId() > 0)
			{
				...
				{
					if((userDto.getUserName().equals(userDto.getUserName().toLowerCase()))
							&& (userDto.getUserPassword().equals(userDto.getUserPassword().toLowerCase())))
					{
						...
						if(t)
						{
							...
							userEntity.setUserPassword(userDto.getUserPassword().toLowerCase());
					...
					{
						 ObjectError error = new ObjectError("globalError", "Username and password should be in smaller case letter's only!");
					     result.addError(error);
	...
	public String loginUser(UserDto userDto, BindingResult result, Model model, HttpSession session) 
	{
				...
				&& (null !=userDto.getUserPassword() && !userDto.getUserPassword().equalsIgnoreCase(""))) 
		{
			if((userDto.getUserName().equals(userDto.getUserName().toLowerCase()))
					&& (userDto.getUserPassword().equals(userDto.getUserPassword().toLowerCase())))
			{
				UserEntity userEntity = userRepository.loginUser(userDto.getUserName().toLowerCase(), userDto.getUserPassword().toLowerCase());
